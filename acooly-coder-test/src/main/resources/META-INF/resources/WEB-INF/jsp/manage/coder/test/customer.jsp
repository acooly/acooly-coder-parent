...
<div class="easyui-layout" data-options="fit : true,border : false">
        ...
        <tr>
          <td align="left">
          	<div>
				...
				性别: <select style="width:80px;height:27px;" name="search_EQ_gender" editable="false" panelHeight="auto" class="easyui-combobox"><option value="">所有</option><c:forEach var="e" items="${allGenders}"><option value="${e.key}" ${param.search_EQ_gender == e.key?'selected':''}>${e.value}</option></c:forEach></select>
					姓名: <input type="text" class="text" size="15" name="search_LIKE_realName"/>
				证件类型: <select style="width:80px;height:27px;" name="search_EQ_idcardType" editable="false" panelHeight="auto" class="easyui-combobox"><option value="">所有</option><c:forEach var="e" items="${allIdcardTypes}"><option value="${e.key}" ${param.search_EQ_idcardType == e.key?'selected':''}>${e.value}</option></c:forEach></select>
				...
				客户类型: <select style="width:80px;height:27px;" name="search_EQ_customerType" editable="false" panelHeight="auto" class="easyui-combobox"><option value="">所有</option><c:forEach var="e" items="${allCustomerTypes}"><option value="${e.key}" ${param.search_EQ_customerType == e.key?'selected':''}>${e.value}</option></c:forEach></select>
  ...
  <div data-options="region:'center',border:false">
    <table id="manage_customer_datagrid" class="easyui-datagrid" url="${pageContext.request.contextPath}/manage/coder/test/customer/listJson.html" toolbar="#manage_customer_toolbar" fit="true" border="false" fitColumns="false"
      ...
      <thead>
        <tr>
			...
			<th field="salary" sum="true">薪水</th>
    ...
    <div id="manage_customer_action" style="display: none;">
      <a onclick="$.acooly.framework.edit({url:'/manage/coder/test/customer/edit.html',id:'{0}',entity:'customer',width:500,height:400});" href="#" title="编辑"><i class="fa fa-pencil fa-lg fa-fw fa-col"></i></a>
      <a onclick="$.acooly.framework.show('/manage/coder/test/customer/show.html?id={0}',500,400);" href="#" title="查看"><i class="fa fa-file-o fa-lg fa-fw fa-col"></i></a>
    ...
    <div id="manage_customer_toolbar">
      <a href="#" class="easyui-linkbutton" plain="true" onclick="$.acooly.framework.create({url:'/manage/coder/test/customer/create.html',entity:'customer',width:500,height:400})"><i class="fa fa-plus-circle fa-lg fa-fw fa-col"></i>添加</a>
