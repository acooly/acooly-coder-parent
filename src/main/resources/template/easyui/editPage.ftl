      ...
      <jodd:form bean="${entityVariable}" scope="request">
		...
		<#list table.columns as entity>
			...
			<tr>
				...
				<td><input type="text" name="${entity.propertyName}" size="20" value="<fmt:formatDate value="${r"${"}${entityVariable}.${entity.propertyName}}" pattern="yyyy-MM-dd HH:mm:ss"/>" onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" <#if !entity.nullable>data-options="required:true"</#if> /></td>
				<#elseif entity.dataType.number>
				<td><input type="text" name="${entity.propertyName}" size="48" class="easyui-numberbox" <#if !entity.nullable>data-options="required:true"</#if> validType="byteLength[1,${entity.length}]"/></td>
				<#else>
				...
				<#else>
				<td><input type="text" name="${entity.propertyName}" size="48" class="easyui-validatebox" <#if !entity.nullable>data-options="required:true"</#if> <#if entity.length gt 0>validType="byteLength[1,${entity.length}]"</#if>/></td>
				</#if>
				</#if>
				</#if>
			</tr>					
